...
public:
  ...
  initialize(const std::string& instanceId, const std::string& timestampFormat)
  {
    ...
    m_logLocation = "";
    if (const char* envVar = std::getenv("NDN_HELLOWORLD_LOGFOLDER"); envVar != nullptr) {
      m_logLocation = envVar;
    }
    if (m_logLocation.empty()) {
      std::cout << "Environment variable NDN_HELLOWORLD_LOGFOLDER not set.\n"
    ...
    if (std::filesystem::exists(logdir)) {
      if (std::filesystem::is_directory(logdir)) {
        ...
        std::cout << "NDN_HELLOWORLD_LOGFOLDER is not a directory.\n"
                  << "Using default output for logging." << std::endl;
        m_logLocation = "";
      }
    }
    else {
      std::cout << "NDN_HELLOWORLD_LOGFOLDER does not exist.\n"
                << "Using default output for logging." << std::endl;
      m_logLocation = "";
    }
  }
